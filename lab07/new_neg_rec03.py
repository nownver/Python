# overlap - handle negative width/height
import turtle as t
...
class Rectangle():
    ...
    def intersect(self, other):
        ...
        if self.width < 0:
            range_x_self = set(range(self.width + self.x, self.x))
        if self.height < 0:
            range_y_self = set(range(self.y, self.y + abs(self.height)))

        if other.width < 0:
            range_x_other = set(range(other.x + other.width, other.x))
        if other.height < 0:
            range_y_other = set(range(other.y, other.y + abs(other.height)))
        ...
        if len(intersect_x) != 0 and len(intersect_y) != 0:
            w = (max(intersect_x) - min(intersect_x))
            h =(max(intersect_y) - min(intersect_y))
            return Rectangle(min(intersect_x), max(intersect_y), w, h) 
        
        else:
            print(set(range(self.width + self.x, self.x)))
